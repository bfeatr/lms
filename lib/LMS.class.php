...
function MacExists($id, $table)
 	{
 		switch($table)
 		{
 			case 0:
 			    return $this->DB->GetOne('SELECT * FROM woj_mac_zmiany WHERE id = ?', array($id));
 			    break;
 			case 2:
 			    return $this->DB->GetOne('SELECT * FROM woj_mac_arch WHERE id = ?', array($id));
 			    break;
 			case 1:
 			default:
 			    return $this->DB->GetOne('SELECT * FROM woj_mac_ok WHERE id = ?', array($id));
 		}
 	}
 
 	function GetMac($id, $table)
 	{
 		switch($table)
 		{
 			case 0:
 			    return $this->DB->GetRow('SELECT id, inet_ntoa(ip) AS ip, mac, port, vlan, FROM_UNIXTIME(lastonline) AS lastonline FROM woj_mac_zmiany WHERE id = ?', array($id));
 			    break;
 			case 2:
     			    return $this->DB->GetRow('SELECT id, inet_ntoa(ip) AS ip, mac, port, vlan, FROM_UNIXTIME(lastonline) AS lastonline FROM woj_mac_arch WHERE id = ?', array($id));
 			    break;
 			case 1:
 			default:
 			    return $this->DB->GetRow('SELECT id, inet_ntoa(ip) AS ip, mac, port, vlan, FROM_UNIXTIME(lastonline) AS lastonline FROM woj_mac_ok WHERE id = ?', array($id));
 		}
 	}
 
 	function DeleteMac($id, $table)
 	{
 		switch($table)
 		{
 			case 0:
 			    return $this->DB->Execute('DELETE FROM woj_mac_zmiany WHERE id = ?', array($id));
 			    break;
 			case 2:
 			    return $this->DB->Execute('DELETE FROM woj_mac_arch WHERE id = ?', array($id));
 			    break;
 			case 1:
 			default:
 			    return $this->DB->Execute('DELETE FROM woj_mac_ok WHERE id = ?', array($id));
 		}
 	}
 
 	function DeleteMacList($table=0, $search=NULL, $sqlskey='AND')
 	{
 		if(sizeof($search))
 		{
 			foreach($search as $key => $value)
 			{
 				...
 				if($value!='')
 				{
 					...
 					switch($key)
 					{
 						...
 						break;
 						case 'port':
 							$searchargs[] = "(port ?LIKE? $value2)";
 						break;
 						case 'vlan':
 							$searchargs[] = "(vlan ?LIKE? $value2)";
 						break;
 						case 'from':
 							$searchargs[] = "(lastonline >= UNIX_TIMESTAMP($valoryg))";
 						break;
 						case 'to':
 							$searchargs[] = "(lastonline <= UNIX_TIMESTAMP($valoryg))";
 						break;
 		...
 		{
 		    case 0: 
 			$sqltable = 'woj_mac_zmiany';
 			break;
 		    case 2:
 			$sqltable = 'woj_mac_arch';
 			break;
 		    case 1:
 		    default:
 			$sqltable = 'woj_mac_ok';
 	...
 	function UpdateMac($mac, $ip, $port, $vlan, $lastonline)
 	{
 		$this->DB->Execute('DELETE FROM woj_mac_zmiany WHERE mac = ? AND ip = inet_aton(?)', array($mac, $ip));
 		return $this->DB->Execute('UPDATE woj_mac_ok SET port = ?, vlan = ?, lastonline = UNIX_TIMESTAMP(?) WHERE mac = ? AND ip = inet_aton(?)', array($port, $vlan, $lastonline, $mac, $ip));
 	}
 
 	function ArchMac($id, $table)
 	{
 		$maclist = $this->GetMac($id, $table);
 		
 		$this->DB->Execute('INSERT INTO woj_mac_arch VALUES(NULL, inet_aton(?), ?, ?, ?, UNIX_TIMESTAMP(?))', array($maclist['ip'],$maclist['mac'],$maclist['port'],$maclist['vlan'],$maclist['lastonline']));
 	...
 	function GetMacList($order='mac,asc',$table=0, $search=NULL, $sqlskey='AND', $details, $limit)
 	{
 		...
 		$maclist = $this->DB->GetAll(
 				'SELECT id, inet_ntoa(ip) AS ip, mac, port, vlan, FROM_UNIXTIME(lastonline) AS lastonline '
 				.'FROM '.$sqltable.' '
 				.'WHERE 1=1 '
 				.($sqlsarg !='' ? ' AND ('.$sqlsarg.')' :'')
 				.($sqlord !='' ? $sqlord.' '.$direction:'')
...
function GetDeviceListS($order='model,asc', $search=NULL, $sqlskey='AND')
	{
		if($order=='')
			$order='model,asc';
	...
	function GetDeviceList($order='model,asc')
	{
		list($order,$direction) = sscanf($order, '%[^,],%s');
	...
	function CustomerDeviceAdd($id,$deviceid)
	{
		$this->DB->Execute('UPDATE woj_device SET customerid=? WHERE id=?', array($id, $deviceid) );
	}

	function CustomerDeviceDelete($deviceid)
	{
...
//			return $this->DB->GetLastInsertID('woj_device');
 ...
 function GetDeviceListB($order = 'model,asc', $search = NULL, $sqlskey = 'AND') {
		if ($order == '')
			$order = 'model,asc';
